...
All stochastic code (placebo-date randomization, synthetic event-panel
generation) must draw from a generator produced here, never from the global
``numpy.random`` state, so that a single master seed reproduces an entire run.
We use the PCG64 bit generator (numpy's default, statistically robust) and
derive independent substreams via :meth:`numpy.random.SeedSequence.spawn`.
